...
class InventoryManager {
  ...
  /**
   * Returns quantity available for purchase.
   *
   * BUG 1: Counts expired reservations as still "reserved", understating
   * availability. Expired reservations should be ignored here (or cleaned up
   * before reading). A 15-min-old unpaid reservation permanently blocks stock.
   */
  getAvailable(productId) {
    ...
    for (const [, res] of this.reservations) {
      if (res.productId === productId) {
        reserved += res.quantity; // missing: && res.expiresAt > Date.now()
      }
    }
...
class PricingEngine {
  ...
  /**
   * BUG 2: Volume discount is based on total item count across the whole cart
   * (1 item of qty 10 = same as 10 different items of qty 1). Business intent
   * is per-line-item volume discount, but the implementation conflates
   * quantity across unrelated products, over-discounting mixed carts.
   */
  ...
  /**
   * BUG 3: Promo-code use count is incremented here, before the order is
   * confirmed or payment succeeds. Failed or abandoned checkouts permanently
   * consume uses, exhausting codes prematurely.
   */
  applyPromoCode(subtotal, code) {
    ...
    promo.uses++; // should happen after successful payment
...
class OrderProcessor {
  ...
  async createOrder(customerId, items, customerTier, promoCode, paymentMethod, shippingAddress) {
    ...
    const order = {
      ...
      createdAt:     Date.now(),
      /**
       * BUG 4: Loyalty points earned on `total` (which includes tax and
       * shipping), not on `subtotal`. Customers earn points on money that
       * goes to the carrier and tax authority, inflating loyalty balances.
       */
  ...
  /**
   * BUG 5: The return value of payment.refund() is never checked.
   * If the refund fails (gateway error, already refunded, etc.) the order is
   * still flipped to 'cancelled' and the customer loses their money.
   */
  async cancelOrder(orderId, reason) {
    ...
    await this.payment.refund(order.transactionId, order.total); // result ignored
  ...
  /**
   * BUG 6: Page 1 skips the first `pageSize` records.
   * `start = page * pageSize` with page=1,pageSize=10 → start=10, not 0.
   * Correct formula is `(page - 1) * pageSize`.
   */
  getOrderHistory(customerId, page = 1, pageSize = 10) {
    const all = [...this.orders.values()]
      .filter(o => o.customerId === customerId)
      .sort((a, b) => b.createdAt - a.createdAt);
    const start = page * pageSize; // off-by-one: should be (page - 1) * pageSize
...
class CustomerAccountManager {
  ...
  async login(email, password) {
    ...
    /**
     * BUG 7: expiresAt is stored in Unix seconds but validateSession compares
     * it against Date.now() which returns milliseconds. The stored value
     * (~1.7 × 10^9) is always less than the current ms timestamp (~1.7 × 10^12),
     * so every session fails validation the instant it is created.
     */
    this.sessions.set(token, {
      customerId: account.id,
      email,
      expiresAt: Math.floor(Date.now() / 1000) + 3600, // seconds — should be ms
    });
  ...
  /**
   * BUG 8: Loyalty points are worth 1 cent each (100 points = $1), but the
   * discountAmount returned is the raw point count, not dollars.
   * The caller applies it as a dollar discount, giving 100× too much off.
   */
  redeemLoyaltyPoints(customerId, points) {
    for (const [, acc] of this.accounts) {
      if (acc.id === customerId) {
        ...
        acc.loyaltyPoints -= points;
        this._updateTier(acc);
        return { success: true, discountAmount: points }; // should be points / 100
      }
    }
...
class ReviewSystem {
  ...
  /**
   * BUG 9: No duplicate-review guard. A customer can submit unlimited reviews
   * for the same product. Each one is stored under a fresh ID, inflating the
   * average rating and polluting the review list.
   */
  ...
  getProductRating(productId) {
    ...
    /**
     * BUG 10: Rounds to the nearest integer. A product with ratings 4, 4, 5
     * returns 4 instead of 4.33. Displayed stars are consistently wrong, and
     * products can't be meaningfully sorted by rating.
     */
    return Math.round(total / ids.size); // should return total / ids.size
  }

  /**
   * BUG 11: Sorts ascending (least helpful first), so getTopReviews returns
   * the reviews that users found least useful — the opposite of the intent.
   */
  getTopReviews(productId, limit = 5) {
    ...
    return [...ids]
      ...
      .sort((a, b) => a.helpful - b.helpful) // should be b.helpful - a.helpful
      .slice(0, limit);
  }
...
class CartService {
  ...
  /**
   * BUG 12: Promo code is stored on the cart without any validation.
   * Invalid, expired, or fully-used codes are silently accepted and shown as
   * "applied" in the UI. The code is only rejected later at checkout, but the
   * cart summary already shows a (false) discounted price.
   */
  applyPromoCode(sessionId, code) {
    const cart = this._getOrCreate(sessionId);
    cart.promoCode = code; // no validation — should call pricing.applyPromoCode first
